#!/usr/bin/env python3
"""
Fixed version of Qwen training script with better progress tracking and tokenization.
"""
...
class QwenForteanTrainer:
    ...
    def prepare_fortean_dataset(self, data_path: Path) -> DatasetDict:
        ...
        train_texts = [format_fortean_instruction(ex) for ex in train_data]
        val_texts = [format_fortean_instruction(ex) for ex in val_data]
    ...
    def train(self, dataset: DatasetDict, output_dir: Path, num_epochs: int = 6):
        """Train with settings optimized for M4 Pro"""
